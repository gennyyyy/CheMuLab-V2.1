...
const DiscoveryService = {
    ...
    mergeUserData(local, remote) {
        ...
        return {
            ...
            discoveries: this.deduplicateDiscoveries([...(local.discoveries || []), ...(remote.discoveries || [])])
        };
    },

    // Deduplicate discoveries by element ID
    deduplicateDiscoveries(discoveries) {
        const seen = new Set();
        return discoveries.filter(discovery => {
            const key = discovery.id || discovery.element;
            if (seen.has(key)) {
                return false;
            }
            seen.add(key);
    ...
    updateProgress(username, userData = null) {
        ...
        // Discoveries that exist in the array are completed discoveries
        const completedDiscoveries = userData.discoveries ? userData.discoveries.length : 0;
        const progressPercentage = (completedDiscoveries / totalPossibleDiscoveries) * 100;

        userData.progress = {
            ...userData.progress,
            totalDiscoveries: userData.discoveries ? userData.discoveries.length : 0,
    ...
    getDiscoveries(userIdOrName) {
        // If we have a current Firebase user, use their uid; otherwise use the parameter
        let id = userIdOrName;
        if (window.firebase && window.firebase.auth) {
            const currentUser = window.firebase.auth().currentUser;
            if (currentUser) {
        ...
        if (!id) {
            console.warn('[DiscoveryService] No user ID provided to getDiscoveries');
            return [];
        }

        const local = localStorage.getItem(this.STORAGE_KEYS.USER_DATA + id);
        const userData = local ? JSON.parse(local) : null;
        const discoveries = userData ? (userData.discoveries || []) : [];
        
        console.log('[DiscoveryService] getDiscoveries:', { id, foundLocal: !!userData, discoveryCount: discoveries.length });
        return discoveries;
    },

    getProgress(userIdOrName) {
        // If we have a current Firebase user, use their uid; otherwise use the parameter
        let id = userIdOrName;
        if (window.firebase && window.firebase.auth) {
            const currentUser = window.firebase.auth().currentUser;
        ...
        if (!id) {
            console.warn('[DiscoveryService] No user ID provided to getProgress');
            return null;
        }

        const local = localStorage.getItem(this.STORAGE_KEYS.USER_DATA + id);
        const userData = local ? JSON.parse(local) : null;
        const progress = userData ? userData.progress : null;
        
        console.log('[DiscoveryService] getProgress:', { id, foundLocal: !!userData, progress });
        return progress;
    },
