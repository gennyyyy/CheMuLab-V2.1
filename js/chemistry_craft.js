...
const ChemistryCraft = {
    ...
    // Small non-blocking loader used for loading/saving discoveries
    startLoading(message = 'Loading...') {
        // Create loader container if missing
        let loader = document.querySelector('.cc-loader');
        if (!loader) {
            loader = document.createElement('div');
            loader.className = 'cc-loader';
            ...
            label.className = 'cc-loader-message';
            loader.appendChild(label);

            document.body.appendChild(loader);
        }
        loader.querySelector('.cc-loader-message').textContent = message;
    ...
    stopLoading(successMessage = '') {
        ...
        if (!loader) return;
        // If a short success message is provided, show it briefly inside the loader area
        if (successMessage) {
            const label = loader.querySelector('.cc-loader-message');
            ...
            // Add a slight delay before hiding so "loading..." is readable
            setTimeout(() => {
                loader.classList.remove('visible');
            }, 800);
        }
    ...
    // Header-saving indicator (cloud-only SVG next to account name)
    showHeaderSaving() {
        // Attach to the user status area if possible
        const target = document.getElementById('userStatus') || document.querySelector('.top-bar');
        if (!target) return;

        let indicator = document.getElementById('cc-header-indicator');
        if (!indicator) {
            indicator = document.createElement('div');
            indicator.id = 'cc-header-indicator';
            indicator.className = 'cc-header-indicator';

            // Cloud SVG icon (black stroke on white background) shown while saving
            indicator.innerHTML = `
                <svg class="cc-header-cloud" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" title="Saving">
                    <path d="M20 17.58A4 4 0 0016 11h-1.26A6 6 0 006 17.5" stroke="#000" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M16 17H7a4 4 0 110-8 5 5 0 019-3 6 6 0 011 11z" stroke="#000" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>`;
            target.appendChild(indicator);
        }

        // Saving state (animated cloud)
        indicator.classList.remove('cc-header-saved');
        indicator.classList.add('cc-header-saving');
        indicator.style.display = 'inline-flex';
    },

    hideHeaderSaving(successMessage = '') {
        const indicator = document.getElementById('cc-header-indicator');
        if (!indicator) return;
        ...
        if (successMessage) {
            ...
            if (!indicator.querySelector('.cc-header-check-overlay')) {
                ...
                checkOverlay.className = 'cc-header-check-overlay';
                checkOverlay.innerHTML = `
                    <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                        <circle cx="12" cy="12" r="10" fill="#8ef0a2" stroke="none"/>
                        <path d="M7 12l3 3 6-6" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>`;
                indicator.appendChild(checkOverlay);
            }
            indicator.style.display = 'inline-flex';
            // Keep indicator visible (don't hide after delay)
        } else {
            // Stop animation but keep visible
            indicator.classList.remove('cc-header-saving');
            indicator.style.display = 'inline-flex';
        }
    },
    ...
    async loadSavedDiscoveries() {
        ...
        try {
            // show loader while we retrieve discoveries
            ...
            if (!currentUser?.uid) {
                ...
                // Do not show a large toast here; stop the compact loader and return.
                this.stopLoading();
                return;
            }
    ...
    async saveDiscoveries() {
        ...
        // Use header indicator while saving (non-blocking)
        ...
        try {
            ...
            // Calculate progress
            const completedDiscoveries = mergedDiscoveries.length;
            ...
            // Save to localStorage and dispatch progress update
            if (window.DiscoveryService) {
                try {
                    // Save discoveries and progress to localStorage
                    const userData = {
            ...
            this.hideHeaderSaving(`${discoveries.length} discoveries saved!`);

        } catch (error) {
            console.error('[ChemistryCraft] Error saving discoveries:', error);
            this.hideHeaderSaving();
